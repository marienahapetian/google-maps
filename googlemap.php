			...
			<ul class="tb_popup_form">
				...
				<li class="has_background">
					...
					<div class="slider-container" style="float:left; width:60%; height:25px; ">
						<input name="map_width" id="map_width" data-slider-highlight="true"
						       ...
						       value="<?php echo $shortcodemap->width; ?>"/>
						<span style="position:absolute; top: -1px; right: 0px;"><?php echo $shortcodemap->width; ?>
							%</span>
					</div>
				</li>
				<li>
				...
				<li class="has_background">
					...
					<select name="map_align" id="map_align">
						<option value="left" <?php if ( $shortcodemap->align == 'left' ) {
							echo 'selected';
						}; ?>>left
						</option>
						<option value="center" <?php if ( $shortcodemap->align == 'center' ) {
							echo 'selected';
						}; ?>>center
						</option>
						<option value="right" <?php if ( $shortcodemap->align == 'right' ) {
							echo 'selected';
						}; ?>>right
						</option>
					</select>
				...
				<li>
					...
					<input type="text" name="map_border_radius" id="map_border_radius"
					       value="<?php echo $shortcodemap->border_radius; ?>"/>
				</li>
			</ul>
		</form>
	</div>
...
function hg_gmaps_api_key_notice(){
	...
	<div id="hg_gmaps_no_api_key_big_notice" class="error">
		...
		<div><a class="hg_mui_btn hg_mui_btn_raised_blue" href="https://console.developers.google.com/flows/enableapi?apiid=maps_backend,geocoding_backend,directions_backend,distance_matrix_backend,elevation_backend&keyType=CLIENT_SIDE&reusekey=true">Register for Google Maps API now</a></div>
